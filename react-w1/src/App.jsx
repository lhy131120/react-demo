							...
							<tbody>
								{products.map((item) => (
									<tr key={item.id}>
										...
										<td>{item.price}</td>
										<td>{item.is_enabled ? "啟用" : "禁止"}</td>
										<td>
											<button className="btn btn-warning" onClick={() => setTempProduct(item)}>
												查看細節
											</button>
										</td>
									</tr>
							...
							<div className="card mb-3">
								...
								<div className="card-body">
									<h5 className="card-title">
										{tempProduct.title}
										<span className="badge bg-danger ms-2">{tempProduct.category}</span>
									</h5>
									...
									<div className="d-flex flex-wrap">
										{Array.isArray(tempProduct.imagesUrl) ? (
											tempProduct.imagesUrl.map((imageUrl, index) => (
												<div key={`image_${index + 1}`} data-key={`image_${index + 1}`} style={{display: "inline-block"}}>
													<img
														className="img-fluid"
											...
											<p className="text-secondary">沒有圖片</p>
										)}
									</div>
								</div>
							</div>
